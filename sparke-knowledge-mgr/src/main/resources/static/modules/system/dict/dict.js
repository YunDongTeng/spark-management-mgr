...
Dict.initColumn = function () {
    return [
        ...
        {title: '排序', field: 'sort', align: 'center', visible: false, valign: 'middle', sortable: true}
    ]
        ;
};
...
Dict.openAddDict = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/system/dict/dict_add'
    });
...
Dict.openDictDetail = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/system/dict/dict_edit/' + Dict.seItem.id
        });
        this.layerIndex = index;
    }
};
...
Dict.delete = function () {
    if (this.check()) {
        ...
        var operation = function () {
            var ajax = new $ax(Feng.ctxPath + "/system/dict/delete", function (data) {
                Feng.success("删除成功!");
                Dict.table.refresh();
            }, function (data) {
        ...
        Feng.confirm("是否刪除字典 " + Dict.seItem.type + "?", operation);
    }
};
...
$(function () {
    ...
    var table = new BSTable(Dict.id, "/system/dict/list", defaultColunms);
