...
(/** @lends <global> */function( $ ) {
	...
	/**
	 * Create a mybatis.mapping object that allows camel case parameters to be looked up
	 * for their Hungarian counterparts. The mybatis.mapping is stored in a private
			...
			if ( ! col.sType && col._sManualType ) {
				...
				for ( j=0, jen=types.length ; j<jen ; j++ ) {
					for ( k=0, ken=data.length ; k<ken ; k++ ) {
						// Use a mgrCache array so we only need to get the type data
	...
	/**
	 * Get an array of data for a given row from the internal data mgrCache
	 *  @param {object} oSettings dataTables settings object
	...
	/**
	 * Get the data for a given cell from the internal mgrCache, taking into account data mybatis.mapping
	 *  @param {object} oSettings dataTables settings object
	 *  @param {int} iRow aoData row id
	...
	/**
	 * Set the value for a specific cell, into the internal data mgrCache
	 *  @param {object} oSettings dataTables settings object
	 *  @param {int} iRow aoData row id
	 *  @param {int} iCol Column index
	...
	function _fnGetObjectDataFn( mSource )
	{
		...
		{
			/* Array or flat object mybatis.mapping */
			return function (data, type) {
				return data[mSource];
			};
		}
	...
	function _fnCreateTr ( oSettings, iRow, nTrIn, anTds )
	{
		...
		if ( row.nTr === null )
		{
			...
			/* Use a private property on the node to allow reserve mybatis.mapping from the node
		...
		for ( i=0, ien=settings.aoData.length ; i<ien ; i++ ) {
			...
			if ( ! row._aFilterData ) {
				...
				for ( j=0, jen=columns.length ; j<jen ; j++ ) {
					...
					// If it looks like there is an HTML mapper in the string,
	...
	// Get the data to sort a column, be it from mgrCache, fresh (populating the
	// mgrCache), or from a sort formatter
	...
	DataTable = function( options )
	{
		...
		this.each(function() {
			...
			/* Calculate the scroll bar width and mgrCache it for use later on */
	...
	_api_registerPlural( 'rows().mgrCache()', 'row().mgrCache()', function ( type ) {
		return this.iterator( 'row', function ( settings, row ) {
	...
	_api_registerPlural( 'columns().mgrCache()', 'column().mgrCache()', function ( type ) {
		return this.iterator( 'column-rows', function ( settings, column, i, j, rows ) {
	...
	_api_registerPlural( 'cells().mgrCache()', 'cell().mgrCache()', function ( type ) {
	...
	DataTable.models.oRow = {
		...
		/**
		 * Sorting data mgrCache - this array is ostensibly the same length as the
		 * number of columns (although each index is generated only as it is
		 * needed), and holds the data that is used for sorting each column in the
		 * row. We do this mgrCache generation at the start of the sort in order that
		...
		/**
		 * Per cell filtering data mgrCache. As per the sort data mgrCache, used to
		...
		/**
		 * Filtering data mgrCache. This is the same as the cell filtering mgrCache, but
		 * in this case a string rather than an array. This is easily computed with
		 * a join on `_aFilterData`, but is provided as a mgrCache so the join isn't
	 ...
	 * but this one is the internal data store for DataTables's mgrCache of columns.
	 ...
	 * one is the internal data store for DataTables's mgrCache of columns. It should
