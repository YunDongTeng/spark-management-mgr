...
// sparkLink 菜单
    _e(function (E, $) {
        ...
        E.createMenu(function (check) {
            ...
            //类型切换事件
            var types = [
                {jumpType: 2 ,name:"活动", pageObj:'Activity', listUrl:'/community/activity', id:"id", title:"title"},
                {jumpType: 3 ,name:"资讯",pageObj:'CommunityNews', listUrl:'/community/news', id:"id", title:"title"}
            ];
            ...
            var $btnSubmit = $('<button class="right">' + lang.submit + '</button>');
            var $btnCancel = $('<button class="right gray">' + lang.cancel + '</button>');
            var $contentNameInput = $('<input type="text" class="block" placeholder="跳转内容" disabled="disabled"/>');
            //选择跳转类型
            ...
            //选择具体内容的id
            var ids = '<input type="hidden" id="sparkLinkContentId" name="类型" style="width: 250px;"/>';
            var $contentIdInput = $(ids);
                    ...
                    for (var i in types){
                        if(typeValue == types[i].jumpType){
                            var index = layer.open({
                                ...
                                fix: false, //不固定
                                maxmin: true,
                                content: Feng.ctxPath + types[i].listUrl,
                                btn: ['确定','取消'],
                                yes: function(index, layero){
                                    //获得选中的id和name
                                    var iframeWin = window[layero.find('iframe')[0]['name']];
                                    if(!iframeWin[types[i].pageObj].check()){
                                        return false;
                                    }
                                    $contentIdInput.val(iframeWin[types[i].pageObj].seItem.id);
                                    $contentNameInput.val(iframeWin[types[i].pageObj].seItem.title);
                                    layer.close(index);
                                }
            ...
            $divIdList.append($contentIdInput);
            $divIdList.append($contentNameInput);
            ...
            menu.clickEvent = function (e) {
                ...
                if (rangeElem ) {
                    ...
                    if(rangeElem.attributes.content){
                        contentId = rangeElem.attributes.content.value;
                        contentName = rangeElem.attributes.contentName.value;
                        if(type==1){
                            url = contentId;
                        }
                    }else{
                ...
                text && $textInput.val(text);
                $typeSelect.val(type);
                contentId && $contentIdInput.val(contentId);
                contentName && $contentNameInput.val(contentName);
                initTypeSelect();
            ...
            $btnSubmit.click(function (e) {
                ...
                var contentId = $.trim($contentIdInput.val());
                var contentName = $.trim($contentNameInput.val());
                ...
                if (!isRangeEmpty) {
                    ...
                    // 执行命令
                    editor.command(e, 'createLink', "javascript:void(0);");
                    ...
                    $newLinks.attr('contentName', contentName);
                    ...
                    commandFn = function () {
                        ...
                        $linkElem.attr('contentName', contentName);
                    ...
                    //内部链接
                    linkHtml = "<a href=\"javascript:void(0);\" type=\""+type+"\" content=\""+contentId+"\" contentName=\""+contentName+"\" onclick=\"spark.contentJump('{\\'type\\':\\'"+type+"\\',\\'content\\':\\'"+contentId+"\\'}')\">" + text + "</a>";
