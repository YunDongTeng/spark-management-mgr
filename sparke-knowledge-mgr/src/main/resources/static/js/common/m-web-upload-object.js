...
(function() {
	...
	var $MyWebUpload = function(pictureId) {
		this.pictureId = pictureId;
		this.uploadBtnId = pictureId + "BtnId";
		this.uploadPreId = pictureId + "PreId";
		this.uploadUrl = Feng.ctxPath + '/system/upload';
    ...
    $MyWebUpload.removeImg=function(id){
        this.pictureId=$("img[id="+id+"]").attr("name");
		$("img[id="+id+"]").remove();
		$("span[id="+id+"]").remove();

        // Feng.success("dataurl[id]"+dataurl[id]);
        var urls="";
		$("img[name="+this.pictureId+"]").each(function(){
            urls+=","+$(this).attr("value");
		})
        urls=urls.substring(1);

		$("#" + this.pictureId).val(urls);
    }

    $MyWebUpload.prototype = {
		/**
		 * 初始化webUploader
		 */
		init : function() {
		...
		bindEvent : function(bindedObj) {
			...
			bindedObj.on('fileQueued', function(file) {
				var $li = $('<div><img id="' + file.id + '" width="100px" height="100px" > <span onclick="$MyWebUpload.removeImg(this.id)" class="cancle glyphicon glyphicon-remove" id="' + file.id + '"></span></div><br>');
				var $img = $li.find('img');

				//判断第一张图片是否是商品的
                var firstImg = $("#"+me.uploadPreId).find('img').first().attr("id");
			...
			bindedObj.on('uploadSuccess', function(file,response) {
                ...
                $("img[id="+file.id+"]").attr("value",response);
                $("img[id="+file.id+"]").attr("name",me.pictureId);

                var urls="";
                var url=$("#" + me.pictureId).val();
                if (url!=null && url!=''){
                     urls=url+","+response;
                }else {
                    urls=response;
				}
				$("#" + me.pictureId).val(urls);
			});
